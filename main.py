...
def main():
    # Komut satırından dosya adını al (Varsayılan değer: test_ekg.csv)
    if len(sys.argv) > 1:
        file_path = sys.argv[1]
    else:
        file_path = "test_ekg.csv"

    # Varsayılan konfigürasyonlar (Argüman kullanmadan çalıştırma için)
    output_dir = "output"
    sampling_rate = 1000
    default_ai = True
    ...
    sampling_rate = getattr(voltage, "attrs", {}).get("sampling_rate", default_ai)
